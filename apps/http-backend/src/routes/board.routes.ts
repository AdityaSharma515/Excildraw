...
import { checkaccess, createBoard, deleteBoard, getBoard, getElements, openBoard, saveElements, saveSnapshots, viewVersions } from "../controllers/board.controllers";
const router:Router=Router();
router.post("/",auth,createBoard);
router.get("/",auth,getBoard);
router.get("/:id",auth,openBoard);
router.delete("/:id",auth,deleteBoard);
router.get("/:id/checkaccess",auth,checkaccess);
